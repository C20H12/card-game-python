...
class Card:
  ...
  def compare(self, other: 'Card') -> int:
    '''
    compare two cards, if their values are the same them compare colors
  ...
  # so that it can be used in built in iterable functions
  ...
  def printCardPretty(self):
    ...
    Colors.printColored(self.color, 
                        f"┃{' ' * spacesPadBefore}{self.color.name}{' ' * spacesPadAfter}┃"
                       )
  ...
  def printCardStr(self):
    '''
    print card as a string in the format COLOR value
    '''
  ...
  def valueOf(self):
    '''
    returns the overall value (ie. how big it is) of the card as a number
    '''
